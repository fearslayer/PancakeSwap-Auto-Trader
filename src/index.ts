...
if (!GLOBAL_CONFIG.PRIVATE_KEY) {
	console.log(blue("The private key was not found in .env. Enter the private key here, or enter it in .env, so as not to enter it again."));
...
console.log( blue("Starting. Amount to Bet:", GLOBAL_CONFIG.AMOUNT_TO_BET, "BNB"),  "\nWaiting for new rounds. It can take up to 5 min, please wait...");

predictionContract.on("StartRound", async (epoch: BigNumber) => {
	
	const d = new Date();
	console.log("\nStarted Epoch", epoch.toString() + " in " + d.getDate().toString() + "/" + d.getMonth().toString() + ", at " + d.getHours().toString() + ":" + d.getMinutes().toString());

	const WAITING_TIME = 275000;

	if(round < 3){
		// Waiting for 300sec
		console.log("Now waiting for", (WAITING_TIME + 25000) / 60000, "min as "+ round.toString() +"° round ends");
		await sleep(WAITING_TIME + 25000);
		round ++;
	}else {
		
		round = 1;	//ripristina il contatore
		console.log("Now waiting for", WAITING_TIME / 60000, "min as " + round.toString() +"° round gets back trade volumes");
		...
		const bearBet = ((bullAmount.gt(bearAmount) && bullAmount.div(bearAmount).lt(5)) || (bullAmount.lt(bearAmount) && bearAmount.div(bullAmount).gt(5))) && Math.random() < 0.8;
	...
	for (let i = 1; i <= 30; i++) {
		...
		console.log("Checking for win at round " + epochToCheck + "");
	...
	if (claimableEpochs.length > 1 && parseInt(claimableEpochs[0].toString()) + 3 < parseInt(epoch.toString())) {
		console.log(green("You have some money to claim"));
		try {
